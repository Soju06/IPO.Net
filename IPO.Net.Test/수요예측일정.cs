namespace IPO.Net.Test {
    public partial class 수요예측일정 : TabActionControl {
        public 수요예측일정() {
            InitializeComponent();
        }
        ...
        async public override void OnRefresh() {
            ...
            var table = await (radioButton1.Checked ?
                Ipo.GetDemandForecastSchedulesAsync(new DateTimeRange(dateTimePicker1.Value, dateTimePicker2.Value)) :
                Ipo.GetDemandForecastSchedulesAsync(new PageRange((int)numericUpDown1.Value - 1, (int)numericUpDown2.Value - (int)numericUpDown1.Value + 1)));
            view.DataSource = table.CreateDataTable();
            view.Columns[1].DefaultCellStyle.Format = "yyyy/MM/dd ~| yyyy/MM/dd";
        }
